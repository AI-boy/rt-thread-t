...
rt_err_t rt_hw_adc_register(rt_adc_device_t device, const char *name,  struct rt_adc_ops *ops, const void *user_data)
{
    rt_err_t result = RT_EOK;
...
rt_err_t rt_device_adc_create(const char *name,  struct rt_adc_ops *ops, const void *user_data)
{
	rt_err_t result = RT_EOK;
	
	rt_adc_device_t device;
	
	device = rt_malloc(sizeof(*device));
	...
	result = rt_hw_adc_register(device, name, ops, user_data);
